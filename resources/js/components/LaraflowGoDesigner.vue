...
export default {
  ...
  mounted() {
    ...
    window.laraflowGo.addDiagramListener("ObjectDoubleClicked", function(e) {
      console.log(e.subject.part.type);
      if (e.subject.part.type.name == "Link") {
        EventBus.$emit(
          "showConfigureCallbacksModal",
          e,
          window.callbackObjects
        );
      }
    });
  ...
  methods: {
    ...
    setLinkLabel(evt) {
      ...
      nodeDataArray.nodeDataArray.forEach(function(element) {
        toNode =
          element.key == evt.subject.part.data.to ? element.text : toNode;
        fromNode =
          element.key == evt.subject.part.data.from ? element.text : fromNode;
      });
      // set the GoJs transition object property
      window.laraflowGo.model.setDataProperty(
        evt.subject.part.data,
        "text",
        fromNode + " to " + toNode
      );
    ...
    configured() {
      ...
      this.callbackMessage =
        "The transition callbacks changed. Please save the workflow before you moving forward.";
    },
